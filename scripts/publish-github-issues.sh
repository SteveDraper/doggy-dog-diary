#!/usr/bin/env bash
# Publish Doggy Dog Diary implementation slices as GitHub issues.
...
ensure_label() {
  ...
  if ! "$GH" label list --repo "$REPO" --json name -q ".[] | select(.name==\"$name\") | .name" | grep -qx "$name"; then
    "$GH" label create "$name" --repo "$REPO" --color "$color" --description "$description" --force
  fi
}

ensure_label afk 0e8a16 "Ready for AFK agent implementation"
ensure_label hitl d876e3 "Requires human decision or review before/during implementation"
...
I1=$(create_issue "Runnable Instance skeleton" "afk" "$(cat <<'EOF'
## What to build

Establish the project skeleton and dev workflow for a local-first **Instance**: FastAPI backend, TypeScript/Tailwind SPA, YAML config, SQLite with migrations, and a single start command. The browser should reach a placeholder home screen; config should create the data directory on first run.

## Acceptance criteria

- [ ] Repo layout includes `backend/`, `frontend/`, `docs/`, and a config example
- [ ] FastAPI serves a health check and the built static SPA
- [ ] SPA scaffold uses TypeScript, Tailwind, and a client-side router
- [ ] YAML config loads `storage_path`, `instance_id`, `host`, and `port`
- [ ] SQLite connection works; migration tooling (Alembic or lightweight equivalent) is set up
- [ ] Single command starts the app (`doggy-dog-diary start` or `make run`)
- [ ] Dev mode uses a Vite proxy to the API
- [ ] Visiting `localhost:8000` shows the placeholder home; config creates the data directory
...
echo "  #$I1 Runnable Instance skeleton"

I2=$(create_issue "Dog Profile and home-screen tiles" "afk" "$(cat <<EOF
## What to build

End-to-end **Dog Profile** management: create and edit dogs with all v1 Profile fields (including kennel club registration and dog status), show them as tiles on the home screen (current dogs first), open a full-screen dog detail view, and support quick-edit overlays for profile fields.

## Acceptance criteria

- [ ] \`dogs\` table and migrations exist
- [ ] CRUD API for dogs covers all v1 Profile fields (name, DOB, sex, breed, neutered, microchip, status, status date, KC registration fields, description)
- [ ] Home screen shows a tile per dog (name; photo placeholder until profile photo slice)
- [ ] Current dogs sort before non-current dogs
- [ ] Dog detail screen shows profile summary
- [ ] Quick-edit overlay updates profile fields without leaving the screen
- [ ] Demo scenario: add Nico and Bella, set status, edit description and kennel club fields
...
echo "  #$I2 Dog Profile and home-screen tiles"

I3=$(create_issue "Profile photo upload and display" "afk" "$(cat <<EOF
## What to build

**Profile photo** upload and display: always stored locally on the backend, shown on the home-screen tile and dog detail Profile.

## Acceptance criteria

- [ ] API accepts profile photo upload for a dog
- [ ] Photos stored under the Instance \`photos/\` directory
- [ ] Home-screen tile shows the profile photo
- [ ] Dog detail shows the profile photo
- [ ] Demo scenario: upload photos for Nico and Bella; tiles display them
...
echo "  #$I3 Profile photo upload and display"

I4=$(create_issue "Household tags and Event foundation" "afk" "$(cat <<EOF
## What to build

Core diary infrastructure: household-scoped **Tags**, **Events** with **Measurements**, and an event detail screen. Enables creating tagged Events for a dog with optional notes and structured measurements.

## Acceptance criteria

- [ ] Tables and migrations for \`events\`, \`event_dogs\`, \`measurements\`, \`tags\`, \`event_tags\`
- [ ] Tag management API (household-scoped, unique names)
- [ ] Event CRUD API with UUID ids and \`created_at\` / \`updated_at\`
- [ ] Single-dog Event create, list, and detail
- [ ] Event detail screen shows notes, tags, and measurements
- [ ] Demo scenario: create a tagged Event for a dog and open its detail screen
...
echo "  #$I4 Household tags and Event foundation"

I5=$(create_issue "Weight View end-to-end" "afk" "$(cat <<EOF
## What to build

**Weight View** for a dog: icon on dog detail, dedicated View screen, quick log overlay for \`weight_lb\`, chronological list, and a basic line chart of weight over time.

## Acceptance criteria

- [ ] Weight View accessible from dog detail (View icon)
- [ ] Log weight overlay creates an Event with \`weight_lb\` measurement
- [ ] List shows weight Events chronologically
- [ ] Line chart plots weight (lb) over date
- [ ] Demo scenario: weight history charts correctly
...
echo "  #$I5 Weight View end-to-end"

I6=$(create_issue "Pooh View end-to-end" "afk" "$(cat <<EOF
## What to build

**Pooh View** for a dog: log \`pooh_size\` (small / medium / large), list Events, and chart pooh history over time.

## Acceptance criteria

- [ ] Pooh View accessible from dog detail
- [ ] Log overlay creates an Event with \`pooh_size\` measurement
- [ ] List shows pooh Events chronologically
- [ ] Chart displays pooh history (count or size over time)
- [ ] Demo scenario: pooh history charts correctly
...
echo ""
echo "Done. Created issues #$I1–#$I26:"
